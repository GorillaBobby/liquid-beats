...
export const Sidebar = () => {
  ...
  useEffect(() => {
    if (user) {
      ...
      loadUnreadCount();
      const channel = supabase.channel('sidebar-messages').on('postgres_changes', {
        event: '*',
        schema: 'public',
        table: 'messages'
      }, () => {
  ...
  return <aside className="hidden md:flex fixed left-0 top-0 h-screen w-64 flex-col bg-glass/50 backdrop-blur-glass border-r border-glass-border z-40">
      <div className="p-6">
        ...
        <nav className="space-y-2">
          ...
          return <Link key={item.name} to={item.href} className={cn("flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-300 relative", isActive ? "bg-gradient-primary text-primary-foreground shadow-glow" : "text-muted-foreground hover:bg-glass-hover hover:text-foreground")}>
                <item.icon className="w-5 h-5" />
                <span className="font-medium">{item.name}</span>
                {item.name === "Messages" && unreadCount > 0 && <span className="absolute top-2 right-2 bg-primary text-primary-foreground rounded-full w-5 h-5 flex items-center justify-center text-xs font-bold">
                    {unreadCount}
                  </span>}
              </Link>;
        })}
      ...
      <div className="mt-auto p-6 border-t border-glass-border">
        <Button onClick={signOut} variant="outline" className="w-full bg-glass/30 border-glass-border hover:bg-glass/50 transition-all duration-300">
